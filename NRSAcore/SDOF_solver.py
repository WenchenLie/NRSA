...
from utils.utils import SDOFError, SDOFHelper, a2u
...
class _SDOF_solver:
    ...
    def run_model(self):
        ...
        # ops.load(2, 1, 0, 0)
        # ops.pattern('Plain', 2, 1, '-fact', -large_m * self.SF)
        # ops.load(2000, 1, 0, 0)
        # 分析
    ...
    def get_responses(self, input_result: tuple[float]) -> Tuple:
        ...
        # 最大绝对加速度
        a_base: float = -ops.nodeAccel(2000, 1)
...
class _SDOF_batched_solver:
    def __init__(self,
        ...
        if not (N_SDOFs == len(ls_T) == len(ls_materials)):
            raise SDOFError(f'SDOF数量、周期数量、材料数量不等！({N_SDOFs}, {len(ls_T)}, {len(ls_materials)})')
    ...
    def run_model(self):
        ...
        # 时程分析
        ops.timeSeries('Path', 1, '-dt', self.dt, '-values', *self.gm, '-factor', self.g)
        for i, ctrlNone in enumerate(self.ctrlNodes):
            # 注：由于同一模型空间下有多个SDOF，因此无法用UniformExcitation施加地震作用。采用  
            #     达朗贝尔原理，将地震动等效为惯性力，对各个SDOF进行不用外力时程作用下的时程分析
            ops.pattern('Plain', i + 1, 1, '-factor', -self.ls_m[i] * self.SF[i])
            ops.load(ctrlNone, 1, 0, 0)
        # 用于读取绝对响应的零刚度SDOF
        large_m = 1e10
        ...
        self.static_node = nodeTag + 1  # 静止节点
        ops.uniaxialMaterial('Elastic', matTag, 0)
        ops.element('zeroLength', eleTag, nodeTag, nodeTag + 1, '-mat', matTag, '-dir', 1, '-doRayleigh', 0)  # 弹塑性
        ops.pattern('Plain', i + 2, 1, '-factor', -large_m)  # 无缩放地震动下静态节点的响应(读取该节点的响应时应乘以对应的缩放系数)
        ops.load(nodeTag + 1, 1, 0, 0)
        nodeTag += 2
        matTag += 1
        eleTag += 1
        # 求解
    ...
    def SDR_tester(self) -> Tuple[tuple[bool], bool]:
        ...
        for i in range(self.N_SDOFs):
            if self.ls_collapse_disp[i] > self.ls_maxAnalysis_disp[i]:
                raise SDOFError('`MaxAnalysisDrift`应大于`CollapseDrift`')
            u = abs(ops.nodeDisp(self.ctrlNodes[i], 1))
    ...
    def get_responses(self, input_result: Tuple[List[float], ...]) -> Tuple[list[float], ...]:
        ...
        for i in range(self.N_SDOFs):
            ...
            u_cent = ls_u_cent[i]
            # 最大相对位移
            u: float = ops.nodeDisp(ctrl_node, 1)
            ls_u.append(u)
            du = u - u_old
            ...
            # 最大绝对加速度
            a_base: float = -ops.nodeAccel(self.static_node, 1) * self.SF[i]
            a_a: float = a_base + ops.nodeAccel(ctrl_node, 1)
            maxAccel[i] = max(maxAccel[i], abs(a_a))
            # 最大基底反力  
            F_total: float = ops.nodeReaction(base_node, 1)
            F_hys = ops.eleForce(ele_tag, 1)
            F_ray = F_total - F_hys
            maxReaction[i] = max(maxReaction[i], abs(F_total))
            ...
            if __name__ == "__main__" and i == 0:
                TIME.append(ops.getTime())
                u_base = -ops.nodeDisp(self.static_node, 1) * self.SF[i]  # 基底位移
                v_base = -ops.nodeVel(self.static_node, 1) * self.SF[i]  # 基底速度
                u_a = u_base + u  # 绝对位移
                v_a = v_base + v  # 绝对速度
                a = ops.nodeAccel(ctrl_node, 1)  # 相对加速度
                A_R.append(a)
                A_A.append(a_a)
                V_R.append(v)
                V_A.append(v_a)
...
class _PDtSDOF_batched_solver:
    ...
    def run_model(self):
        ...
        ops.model('basic', '-ndm', 2, '-ndf', 3)
        nodeTag = 1  # 当前节点编号
        self.baseNodes = []  # 下部基底节点 (inode)
        self.midNodes = []  # 右侧基底节点 (knode)
        self.ctrlNodes = []  # 控制节点 (jnode)
        matTag = 1  # 当前材料编号
        ctrlMats = []  # 控制材料的编号
        eleTag = 1  # 当前单元编号
        self.ctrlEles = []  # 控制单元的编号
        self.stiffEles = []  # 刚性单元的编号
        ...
        for i in range(self.N_SDOFs):
            ...
            ops.region(i + 1, '-ele', eleTag, '-rayleigh', 0, 0, b, 0)  # Rayleigh阻尼
            self.ctrlEles.append(eleTag)
            self.stiffEles.append(eleTag + 1)
            eleTag += 2
        # 用于读取绝对响应的零刚度SDOF
        large_m = 1e10
        ops.node(nodeTag, 0, 0, 0)
        ...
        ops.mass(nodeTag + 1, large_m, 0, 0)
        self.static_node = nodeTag + 1  # 静止节点
        ops.uniaxialMaterial('Elastic', matTag, 1e-9)
        ops.element('zeroLength', eleTag, nodeTag, nodeTag + 1, '-mat', matTag, '-dir', 1, '-doRayleigh', 0)  # 弹塑性
        nodeTag += 2
        matTag += 1
        eleTag += 1
    ...
    def SDR_tester(self):
        ...
        for i in range(self.N_SDOFs):
            if self.ls_collapse_disp[i] > self.ls_maxAnalysis_disp[i]:
                raise SDOFError('`ls_collapse_disp`应大于`ls_maxAnalysis_disp`')
            u = ops.nodeDisp(self.ctrlNodes[i], 1)
    ...
    def get_responses(self, input_result: Tuple[List[float], ...]) -> Tuple[list[float], ...]:
        ...
        for i in range(self.N_SDOFs):
            ...
            maxAccel[i] = max(maxAccel[i], abs(a_a))
            # 最大基底反力
            F_total: float = ops.nodeReaction(base_node, 1) + ops.nodeReaction(mid_node, 1)
            F_hys = ops.eleForce(ele_tag, 1) + ops.eleForce(stiff_ele, 1)
            F_ray = F_total - F_hys
            maxReaction[i] = max(maxReaction[i], abs(F_total))
            ls_F_hys.append(F_hys)
            ls_F_ray.append(F_ray)
            ...
            Ev[i] += Si
            # 最大基底反力
            F: float = -(ops.eleResponse(ele_tag, 'rayleighForces')[0] + ops.eleForce(ele_tag, 1))
...
if __name__ == "__main__":
    ...
    dt = 0.01
    materials = tuple({'Steel01': (Fy, k, alpha)} for _ in range(N_SDOF))
    ...
    with SDOFHelper(suppress=False):
        with VizTracer():
            ...
            results = SDOF_batched_solver(N_SDOF, ls_T, gm, dt, materials, [uy,]*N_SDOF, SF=(1,)*N_SDOF)

            # results = PDtSDOF_batched_solver(N_SDOF, (h,)*N_SDOF, ls_T, ls_grav, gm, dt, PDtMaterials, ls_uy=[uy,]*N_SDOF, ls_SF=(1,)*N_SDOF)
    # print(results)
    # print(state)
    # print(result[8][0])
    resType = A_A
